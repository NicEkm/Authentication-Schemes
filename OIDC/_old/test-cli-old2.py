...
async def post_handler():
    try:
        async with aiohttp.ClientSession() as session: # Creates new session
            ...
            async with session.post('http://localhost:9090/post', data=payload) as resp: # Connects to api-server and send post-request that, is used to debug
                print(resp)
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(post_handler())
    loop.close()
