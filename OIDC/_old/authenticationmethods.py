...
##############################################################################################################
    ...
    async def authentication_code_flow_with_keycloak(self):
        from get_auth_code import Get_Auth_Code # Import get_auth_code module that gets auth_code for given credentials
        ...
        if redirect_url[0:4] == 'http':
            ...
            auth_code = Get_Auth_Code(redirect_url, self.client_id, self.mysecret).Auth_code() # Use get_auth_code module to gain auth_code
            payload = {
                    'auth_code':auth_code,
