...
async def get_id_token(client_identification):
    try:
        await asyncio.sleep(1)
        database = openDatabase()
        for x in database:
...
async def valid_auth_code2(request):
    ...
    if validation() == True:
        ...
        id_token = await get_id_token(client_identification)     
        if id_token != '':
            tokenDict['original_id_token'] = id_token
            json_data = {
                'id_token': id_token,
...
async def handle_authentication4(request):
    from yarl import URL
    import urllib.parse
    ...
    if user_is_legit == True:
        ...
        await asyncio.sleep(1)
        client_identification = authDict['client_id']
        id_token = await get_id_token2(client_identification)
        tokenDict['original_id_token'] = id_token
...
async def get_id_token2(client_identification):
    try:
        await asyncio.sleep(1)
        database = openDatabase()
        for x in database:
...
app.add_routes([
    ...
    web.get('/authorization4', handle_authentication4, name='authorize4'),
    web.post('/oauth/token', valid_auth_code, name='oauth'),
    web.post('/oauth/token2', valid_auth_code2, name='oauthV2')
])
